...
```mermaid
flowchart TD
  A["nvme0n1 whole device 476.9G"] --> B["nvme0n1p1 512M"]
  A --> C["nvme0n1p2 1.7G"]
  A --> D["nvme0n1p3 474.7G"]
  D --> E["cryptroot LUKS encrypted"]
  B --- F["mounted at boot efi"]
  C --- G["mounted at boot"]
  E --- H["mounted at root"]
```
*The lsblk tree: one physical device, three partitions, one encrypted layer on top of the third.*
...
```mermaid
flowchart LR
  A["Partition parted or fdisk"] --> B["Format mkfs.ext4"]
  B --> C["Mount mount command"]
  C --> D["Persist etc fstab"]
  A -.stop here.-> A1["Useless: no filesystem"]
  B -.stop here.-> B1["Invisible: not mounted"]
  C -.stop here.-> C1["Gone after reboot"]
```
*Four steps from raw disk to a durable mount, and what happens if you stop early.*
