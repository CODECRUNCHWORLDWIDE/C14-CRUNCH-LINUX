...
```mermaid
flowchart LR
  P1["PV /dev/sdb1"] --> V["VG data"]
  P2["PV /dev/sdc1"] --> V
  V --> L1["LV web 50 GiB"]
  V --> L2["LV data 200 GiB"]
  L1 --> M1["dev mapper data-web"]
  L2 --> M2["dev mapper data-data"]
```
*Two physical volumes pool into one volume group, which is sliced into independently sized logical volumes.*
...
```mermaid
flowchart TD
  A["Pages clean"] --> B["Write happens pages marked dirty"]
  B --> C{"Dirty ratio reaches background threshold 10 percent"}
  C -->|Yes| D["Background writeback starts kernel flushes async"]
  C -->|No| E["Pages wait age tracked by dirty expire centisecs"]
  D --> F{"Dirty ratio reaches sync threshold 20 percent"}
  F -->|Yes| G["Writer process blocks synchronous writeback"]
  F -->|No| A
  E --> D
```
*How rising dirty-page pressure escalates from background writeback to a synchronous stall.*
