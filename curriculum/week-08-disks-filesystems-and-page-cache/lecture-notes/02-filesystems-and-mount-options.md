...
```mermaid
flowchart TD
  A["Choosing a filesystem"] --> B{"Need snapshots or subvolumes"}
  B -->|Yes| C["btrfs"]
  B -->|No| D{"Large filesystem many parallel writers"}
  D -->|Yes| E["xfs"]
  D -->|No| F{"High random write database"}
  F -->|Yes| G["ext4 or xfs avoid btrfs"]
  F -->|No| H["ext4 the safe default"]
```
*Decision tree for picking ext4, xfs, or btrfs based on the workload.*
...
sequenceDiagram
  ...
  participant Disk as Data blocks on disk
  participant Journal as ext4 journal
  App->>Cache: write file
  Cache->>Disk: flush data blocks first
  Cache->>Journal: commit metadata after data lands
  Note over Disk,Journal: data=ordered keeps this sequence
```
*Why data=ordered writes file data before it commits the matching metadata to the journal.*
