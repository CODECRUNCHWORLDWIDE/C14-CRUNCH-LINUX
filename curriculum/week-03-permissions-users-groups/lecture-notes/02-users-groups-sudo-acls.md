...
erDiagram
    ...
    PASSWD ||--|| SHADOW : "login name"
    PASSWD }o--|| GROUP : "primary gid"
    GROUP ||--|| GSHADOW : "group name"
```
*The four account-database files and how they join to each other.*
...
```mermaid
flowchart TD
    A["User runs sudo command"] --> B["sudo reads etc sudoers and sudoers.d"]
    B --> C{"Rule matches this user or group"}
    C -->|"No match"| D["Permission denied"]
    C -->|"Match found"| E{"NOPASSWD tag present"}
    E -->|"No"| F["Prompt for password"]
    E -->|"Yes"| G["Run command as target user"]
    F --> G
```
*How sudo turns a command plus a sudoers rule into either a password prompt or a straight-through run.*
