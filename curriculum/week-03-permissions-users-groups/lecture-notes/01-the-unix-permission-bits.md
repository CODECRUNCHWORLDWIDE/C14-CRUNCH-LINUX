...
```mermaid
flowchart TD
    A["Process requests access to file"] --> B{"Is process UID the file owner"}
    B -->|"Yes"| C["Apply owner triple"]
    B -->|"No"| D{"Is process GID or a supplementary group the file group"}
    D -->|"Yes"| E["Apply group triple"]
    D -->|"No"| F["Apply other triple"]
```
*The kernel checks owner, then group, then other -- and stops at the first match, even if a later triple would be more permissive.*
...
```mermaid
flowchart TD
    A["Permission denied"] --> B["Run id: confirm your UID GID and groups"]
    B --> C["Run ls -l on the file: check owner group and mode"]
    C --> D["Run ls -ld on the parent directory: check traverse and write bits"]
    D --> E["Diagnosis: which triple or directory bit is blocking you"]
```
*The three-minute method for tracing any permission denied error to its cause.*
