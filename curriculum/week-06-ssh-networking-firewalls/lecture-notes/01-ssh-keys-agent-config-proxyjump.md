...
```mermaid
flowchart TD
  A["TCP connection to port 22"] --> B["Transport layer: key exchange and encryption"]
  B --> C["User-auth layer: prove identity"]
  C --> D["Connection layer: open channels"]
  D --> E["Shell scp forwarding agent channels"]
```
*The three SSH protocol layers stack on a single TCP connection; layer two is where auth decisions live.*
...
sequenceDiagram
  ...
  participant P as Private Host
  L->>B: TCP connect and SSH handshake
  L->>B: Request forward to private host port 22
  B->>P: Open TCP connection
  L->>P: Second SSH handshake end to end
  Note over L,P: Bastion forwards bytes only, cannot decrypt
```
*ProxyJump opens a separate, fully encrypted handshake straight through to the private host; the bastion only relays bytes.*
