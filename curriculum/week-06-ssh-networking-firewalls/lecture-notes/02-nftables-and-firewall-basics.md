...
```mermaid
flowchart LR
  A["Packet arrives at NIC"] --> B{"Destined for this host"}
  B -->|"Yes"| C["input hook"]
  B -->|"No, routed through"| D["forward hook"]
  E["Local process sends packet"] --> F["output hook"]
  C --> G["Delivered to local socket"]
  D --> H["Sent to next hop"]
  F --> H
```
*Netfilter hook points decide whether a packet is delivered locally, forwarded onward, or originates from this host.*
...
```mermaid
stateDiagram-v2
  [*] --> new: First SYN packet
  new --> established: Handshake completes
  established --> related: Related flow opens
  new --> invalid: Malformed or late RST
  invalid --> [*]: Dropped
  established --> [*]: Connection closes
```
*Conntrack classifies every packet into one of these states before nftables applies a verdict.*
