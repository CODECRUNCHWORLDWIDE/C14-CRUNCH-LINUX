...
```mermaid
flowchart TD
  A["Where is X"] --> B["A command binary"]
  A --> C["A program config"]
  A --> D["User settings"]
  A --> E["Logs"]
  B --> B1["which command"]
  C --> C1["etc program directory"]
  D --> D1["dotfile in home or dot config"]
  E --> E1["var log or journalctl"]
```
*The instinctive map for finding anything on a Linux system.*
