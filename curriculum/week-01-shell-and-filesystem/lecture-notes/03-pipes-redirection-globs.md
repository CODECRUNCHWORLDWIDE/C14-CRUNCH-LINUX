...
```mermaid
flowchart TD
  K["Keyboard"] -->|"stdin"| P["Program"]
  P -->|"stdout"| T1["Terminal success output"]
  P -->|"stderr"| T2["Terminal error output"]
```
*Three separate streams — keeping stdout and stderr apart lets you redirect one without touching the other.*
...
flowchart LR
  ...
  B --> C["wc -l stdin"]
```
*A pipe wires one command's stdout directly into the next command's stdin.*
