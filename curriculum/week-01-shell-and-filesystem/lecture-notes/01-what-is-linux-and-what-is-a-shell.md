...
```mermaid
flowchart TD
  A["Your program"] --> B["C library or language runtime"]
  B --> C["System call"]
  C --> D["Linux kernel"]
  D --> E["Hardware CPU memory disk network"]
```
*Your code rarely talks to hardware directly — it goes through a library, a syscall, then the kernel.*
...
```mermaid
flowchart TD
  A["Type ls -la /etc"] --> B["Parse into tokens"]
  B --> C["Look up ls in PATH"]
  C --> D["Fork new process"]
  D --> E["execve runs ls"]
  E --> F["Shell waits for exit"]
  F --> G["Print result and return to prompt"]
```
*Six steps the shell performs every time you run a command.*
