...
```mermaid
flowchart LR
  R["Pick a resource"] --> U["Utilization - percent busy"]
  R --> S["Saturation - queued work waiting"]
  R --> E["Errors - error events"]
  U --> N["Next resource"]
  S --> N
  E --> N
  N --> R
```
*The USE method loops through Utilization, Saturation, and Errors for every resource in turn.*
...
```mermaid
flowchart TD
  T["Request takes 9 seconds"] --> A["On-CPU executing instructions"]
  T --> B["Off-CPU blocked on IO"]
  T --> C["Off-CPU runnable no free CPU"]
  T --> D["Off-CPU waiting on a lock"]
  T --> E["Off-CPU sleeping intentionally"]
```
*Every second of a request's time falls into one of five buckets, each with its own diagnostic tool.*
