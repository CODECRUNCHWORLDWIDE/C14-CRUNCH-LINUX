...
stateDiagram-v2
  [*] --> Running
  Running --> Sleeping: blocks on event
  Sleeping --> Running: event occurs
  Running --> UninterruptibleSleep: blocks on IO
  UninterruptibleSleep --> Running: IO completes
  Running --> Stopped: SIGSTOP
  Stopped --> Running: SIGCONT
  Running --> Zombie: exits
  Zombie --> [*]: parent calls wait
```
*Process state transitions as seen in the S column of top, htop, and ps.*
...
```mermaid
flowchart LR
  A["vmstat 1 shows r at 5 and us at 99 percent"] --> B["mpstat per CPU shows all cores at 99 percent"]
  B --> C["pidstat shows two worker processes at 400 percent CPU"]
  C --> D["ps shows each worker has 16 threads"]
  D --> E["Diagnosis - parallel workload fully using 8 cores"]
```
*The chain of commands that narrows a CPU-bound incident down to two multi-threaded worker processes.*
