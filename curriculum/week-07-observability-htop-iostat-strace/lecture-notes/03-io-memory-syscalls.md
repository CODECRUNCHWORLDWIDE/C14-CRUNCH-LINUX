...
```mermaid
flowchart TD
  A["Read first second of vmstat 1"] --> B{"si or so non-zero"}
  B -->|"yes"| C["Memory pressure - go to free -h"]
  B -->|"no"| D{"b non-zero or wa high"}
  D -->|"yes"| E["IO saturation - go to iostat"]
  D -->|"no"| F{"r above CPU count or us high"}
  F -->|"yes"| G["CPU saturation - go to mpstat"]
  F -->|"no"| H{"cs very high"}
  H -->|"yes"| I["Thread thrash - check pidstat -w"]
  H -->|"no"| J["System is fine"]
```
*Decision tree for reading the first second of vmstat output.*
...
```mermaid
flowchart LR
  A["vmstat 1 shows b at 3 and wa at 94 percent"] --> B["iostat -xz 1 shows sda at 99 percent util"]
  B --> C["pidstat -d 1 shows python3 reading 98 MB per second"]
  C --> D["proc pid io confirms cold reads from disk"]
  D --> E["Diagnosis - disk saturated by sequential reads"]
```
*The chain of commands that narrows an IO-bound incident down to one process reading a saturated disk.*
