...
| [resources.md](./01-resources.md) | Brendan Gregg's USE method, Linux Performance docs, man pages, BPF references |
| [lecture-notes/01-the-use-method-and-where-to-start.md](./02-lecture-notes/01-the-use-method-and-where-to-start.md) | The USE method; the four pillars; the first-sixty-seconds runbook |
| [lecture-notes/02-cpu-and-process-tools.md](./02-lecture-notes/02-cpu-and-process-tools.md) | `htop`, `top`, `ps`, `pidstat`, `mpstat`; columns and process states |
| [lecture-notes/03-io-memory-syscalls.md](./02-lecture-notes/03-io-memory-syscalls.md) | `iostat`, `vmstat`, `free`, `strace`, `ltrace`, `/proc/<pid>/io`; the IO-and-syscall view |
| [exercises/exercise-01-diagnose-a-busy-cpu.md](./03-exercises/exercise-01-diagnose-a-busy-cpu.md) | Run `yes > /dev/null &`, find it with `htop`/`top`/`pidstat`, kill it |
| [exercises/exercise-02-trace-a-syscall.md](./03-exercises/exercise-02-trace-a-syscall.md) | `strace -c ls`; read the summary; identify the dominant syscall |
| [exercises/exercise-03-iostat-during-dd.md](./03-exercises/exercise-03-iostat-during-dd.md) | Run `iostat -x 1` while `dd` writes; identify the saturated column |
| [exercises/SOLUTIONS.md](./03-exercises/SOLUTIONS.md) | Step-by-step solutions to all three exercises |
| [challenges/challenge-01-write-your-own-mini-htop.py](./04-challenges/challenge-01-write-your-own-mini-htop.py) | A type-hinted Python parser of `/proc` that mimics a slice of `htop` |
| [challenges/challenge-02-detect-noisy-neighbor.md](./04-challenges/challenge-02-detect-noisy-neighbor.md) | Find the noisy process on a busy host with `pidstat`, `iotop`, and `/proc/<pid>/io` |
| [quiz.md](./05-quiz.md) | 10 multiple-choice questions |
| [homework.md](./06-homework.md) | Six practice problems (~6 hours) |
| [mini-project/README.md](./07-mini-project/00-overview.md) | Diagnose a synthetic load; write a full performance-debug report with numbers |
