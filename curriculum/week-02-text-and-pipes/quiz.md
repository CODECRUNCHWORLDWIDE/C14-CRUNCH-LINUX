...
**B** — `pattern { action }` is the model. Every rule is tested against every record.
...
**A** — when a rule has a pattern but no action block, the default action is `{ print }`. This is what makes `awk '/error/' file.txt` work as a `grep` substitute.
...
**B** — `$NF` is the last field; `NF` (no `$`) is the count of fields.
...
**A** — `BEGIN`. `BEGINFILE` is a `gawk` extension.
...
**B** — the associative-array idiom. A is just printing all shells, no counts. C counts total lines.
...
**A** — in place, no backup. On GNU, `-i` with no suffix is "no backup."
...
**B** — BSD `-i` requires a suffix; the empty string `''` means "no backup."
...
**C** — `length(array)` for the key count is `gawk`. POSIX `length()` works on strings only.
...
**B** — JSON has nested structure; `awk` is record-oriented and flat. Use `jq` or Python.
