...
```mermaid
flowchart TD
  A["Read line into pattern space"] --> B["Apply sed commands"]
  B --> C["Print pattern space"]
  C --> D["Clear pattern space"]
  D --> A
```
*The per-line cycle sed repeats for every input line.*
...
```mermaid
flowchart LR
  PS["Pattern space"] -->|h copy| HS["Hold space"]
  PS -->|H append| HS
  HS -->|g copy| PS
  HS -->|G append| PS
  PS -->|x exchange| HS
  HS -->|x exchange| PS
```
*How h, H, g, G, and x move data between pattern space and hold space.*
