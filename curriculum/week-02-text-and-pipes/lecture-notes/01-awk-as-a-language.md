...
```mermaid
flowchart TD
  A["Read record"] --> B["Split into fields by FS"]
  B --> C{"Pattern matches"}
  C -->|Yes| D["Run action"]
  C -->|No| A
  D --> A
```
*The read-split-match-act cycle awk repeats for every input record.*
...
```mermaid
flowchart LR
  A["BEGIN runs once"] --> B["Pattern action rules run per record"] --> C["END runs once"]
```
*BEGIN and END bracket the per-record loop as entry and exit hooks.*
