...
```mermaid
flowchart LR
  A["Ctrl-C pressed"] --> B["Kernel sends SIGINT"]
  B --> C["on_interrupt handler runs"]
  C --> D["exit 130 called"]
  D --> E["EXIT trap fires"]
  E --> F["cleanup runs"]
```

*The chain of events from a keypress to cleanup actually running.*
...
```mermaid
flowchart TD
  A["main starts"] --> B["parse flags"]
  B --> C["acquire flock lock"]
  C --> D["create TMPDIR via mktemp"]
  D --> E["register cleanup trap on EXIT"]
  E --> F["do the work"]
  F --> G["EXIT trap runs cleanup"]
```

*Execution order inside the template's main function, lock first, trap before work.*
