...
```mermaid
flowchart TD
  A["New service unit"] --> B{"Sends sd_notify ready signal"}
  B -->|"Yes"| C["Type equals notify"]
  B -->|"No"| D{"Runs once and exits"}
  D -->|"Yes"| E["Type equals oneshot"]
  D -->|"No"| F["Type equals exec"]
```
*Picking the right `Type=` starts with what the service can tell systemd about its own readiness.*
...
sequenceDiagram
  participant Client
  participant systemd
  participant EchoService as echo.service
  systemd->>systemd: Open listening socket at boot
  Client->>systemd: Connect to port 2222
  systemd->>EchoService: Start service and hand off socket
  EchoService->>Client: Handle connection
  Client->>systemd: Next connection
  systemd->>EchoService: Pass to running service
```
*The socket lives before the service does; the first connection is what wakes it up.*
