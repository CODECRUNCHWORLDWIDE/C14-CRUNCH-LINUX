...
```mermaid
flowchart LR
  A["Service stdout and stderr"] --> B["systemd-journald"]
  B --> C["Structured journal database"]
  C --> D["journalctl with filters"]
  D --> E["Human readable or JSON output"]
```
*Every log line takes the same path: captured by journald, stored as a record, filtered back out by journalctl.*
...
```mermaid
flowchart TD
  A["Add one directive"] --> B["daemon-reload"]
  B --> C["Restart service"]
  C --> D{"Service still works"}
  D -->|"No"| F["Relax that one directive"]
  F --> C
  D -->|"Yes"| E["Check exposure score"]
  E --> G{"Score in target range"}
  G -->|"No"| A
  G -->|"Yes"| H["Done"]
```
*Hardening is an iterative ratchet, not a one-shot edit — one directive at a time, verified before the next.*
